...
namespace OriAscendant.UI
{
    ...
    ///
    /// Normal mode: the silhouette kindles from darkness (0→1 over KindleDuration),
    /// then the proverb and tap-prompt fade in after RevealDelay. A tap at any
    /// point calls Skip(), which flags IsDone so the skin can start its own
    /// close-transition.
    ///
    /// Reduce Motion: all three elements fade in together over ReduceMotionFadeDuration
    ...
    public struct ColdOpenBeat
    {
        ...
        /// (silhouetteAlpha, proverbAlpha, promptAlpha) to apply this frame.
        /// All values are in [0, 1]. When IsDone, returns (0, 0, 0) — the skin
        /// should stop applying beat values and run its own close transition instead.
        /// </summary>
        public (float silhouette, float proverb, float prompt) Tick(float dt, bool reduceMotion)
        {
            if (_done) return (0f, 0f, 0f);
            ...
            return (silhouette, reveal, reveal);
        }
    }
}
